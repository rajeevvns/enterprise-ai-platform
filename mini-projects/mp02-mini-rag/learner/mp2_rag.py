...
openai = OpenAI(base_url=os.environ.get("OPENAI_BASE_URL"))
qdrant = QdrantClient(
...
def chunk_document(doc: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if paragraphs and paragraphs[0] == doc["title"]:
    ...
    for para in paragraphs:
        is_section_header = len(para) < 60 and not para.endswith((".", "!", "?", '"'))
        ...
        if buffer and len(buffer) + len(para) + 2 > TARGET_CHUNK_SIZE:
            flush()
            words = buffer.split()
            ...
            buffer = " ".join(reversed(overlap_words))
...
def retrieve(query: str, k: int = 3) -> list[dict[str, Any]]:
    ...
    hits = qdrant.query_points(collection_name=COLLECTION_NAME, query=vector, limit=k).points
